# -*- coding: utf-8 -*-
"""
Created on Wed Jan  6 10:37:02 2021
...
from Example import ExampleSet,graph_collate_fn
from logger import *
...
def run_training(model, train_loader, valid_loader, valset, hps, train_dir):
    ...
    for epoch in range(1, hps.n_epochs + 1):
        ...
        for i, (G, index,item) in enumerate(train_loader):
            ...
            try:
                sim_mat=np.zeros([len(G.ndata["embedding"]),len(G.ndata["embedding"])])
                for i in range(len(G.ndata["embedding"])):
                    for j in range(len(G.ndata["embedding"])):
                        if i!=j:
                            sim_mat[i][j]=cosine_similarity(G.ndata["embedding"][i].reshape(1,-1),G.ndata["embedding"][j].reshape(1,-1))[0,0]
                            nx_graph=nx.from_numpy_array(sim_mat)
            except:
                print("not have done")
                weight=np.random.uniform(0,1,10).tolist()
            else:
                scores=nx.pagerank(nx_graph)
                weight=list(scores.values())
            ...
            label = G.ndata["label"].sum(-1)  # [n_nodes]
            G.ndata["loss"] = criterion(outputs, label).unsqueeze(-1)  # [n_nodes, 1]
...
def main():
    ...
    if hps.model == "HSG":
        ...
        del dataset
        valid_dataset = ExampleSet(VALID_FILE)
